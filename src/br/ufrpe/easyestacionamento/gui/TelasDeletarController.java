...
public class TelasDeletarController {
	...
	public void deletarVeiculo(ActionEvent event) throws IOException, PlacaNaoExisteException, NumberFormatException, CpfNaoExisteException {
		try {
			...
			if (!(placA.equals(""))) {
				...
				Alert alert = new Alert(AlertType.INFORMATION, "Ve�culo removido com sucesso!");
				alert.setTitle("Ve�culo removido");
				alert.setHeaderText("Removido");
				alert.showAndWait();
			} else {
				Alert alert = new Alert(AlertType.ERROR, "Informe uma Placa para remover um ve�culo!");
				alert.show();
			}
			...
			alert.setHeaderText("Tente novamente");
			alert.setContentText("N�o existe um ve�culo cadastrado com essa Placa!");
			alert.showAndWait();
		} finally {
			limparCamposVeiculo();
		}
	...
	public void deletarFuncionario(ActionEvent event) throws IOException, CpfNaoExisteException {
		try {
			if (!(cpfFuncionario.getText().toString().equals(""))) {
				...
				Alert alert = new Alert(AlertType.INFORMATION, "Funcion�rio removido com sucesso!");
				alert.setTitle("Funcion�rio removido");
				alert.setHeaderText("Removido");
				alert.showAndWait();
			} else {
				Alert alert = new Alert(AlertType.ERROR, "Informe um CPF para remover!");
			...
			alert.setTitle("Falha ao deletar");
			alert.setHeaderText("Tente novamente");
			alert.setContentText("N�o existe um funcion�rio cadastrado com esse CPF!");
			alert.showAndWait();
		} catch (NumberFormatException nfe) {
			Alert alert = new Alert(AlertType.INFORMATION);
			alert.setTitle("Falha ao remover");
	...
	public void deletarEstacionamento(ActionEvent event) throws IOException, EstacionamentoNaoExisteException {
		try {
			...
			alert.setTitle("Falha ao deletar");
			alert.setHeaderText("Tente novamente");
			alert.setContentText("N�o existe um estacionamento cadastrado com esse Nome!");
			alert.showAndWait();
		} finally {
